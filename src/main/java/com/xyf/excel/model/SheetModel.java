...
public class SheetModel extends ExcelBaseModel {
	...
	public void switchOperational(OperationalModel operationalModel) {
		// 由列切换至行模式下
		if (getOperationalModel() == OperationalModel.COLUMN && operationalModel == OperationalModel.ROW) {
			this.rowNum = overRowNum;
			this.overRowNum = 0;
		} else {
			this.overRowNum = Integer.valueOf(this.rowNum);
		}
	...
	public void setOverRowNum(int overRowNum) {
		if (this.overRowNum < overRowNum) {
			this.overRowNum = overRowNum;
		}
	}
