...
//import com.chinatechstar.component.commons.entity.ExcelImg;
...
/**
 * 读取Excel文件中图片信息并保存到ExcelImg对象列表中
 */
public class ReadExcelImg {
	 ...
	 */
	public static List<ExcelImg> importAndSaveImages(String excelFilePath, String filename, String path) throws Exception {
		...
		try (XSSFWorkbook workbook = new XSSFWorkbook(fi)) {
			...
			for (XSSFShape shape : drawing.getShapes()) {
				...
				if (shape instanceof XSSFPicture) {
					...
					// 保存图片到指定路径
					String filePath = saveImage(imgData, getPictureExtension(picture), path);
					excelImg.setPath(filePath);
					// 将ExcelImg对象添加到列表中
					list.add(excelImg);
				}
			}
	 ...
	 * @param type    图片类型
	 */
	private static String saveImage(byte[] imgData, String type, String imagePath) throws IOException {
		// 创建保存路径目录（如果不存在）
